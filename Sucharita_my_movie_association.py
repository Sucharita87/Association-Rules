# -*- coding: utf-8 -*-
"""
Created on Tue Jun 23 19:41:54 2020
...
movie= pd.read_csv("F:\\ExcelR\\Assignment\\Association rules\\my_movies.csv")
movie.head(5)
movie.columns # give column names in the dataframe
movie= movie.iloc[:,6:15] # removing columns without dummies
movie.head(2)


freq_item = apriori(movie, min_support = 0.005, max_len = 3, use_colnames = True) 
f= freq_item.sort_values("support", ascending = False) # sort the dataset based on support value

plt.bar(x= list(range(0,6)),height=f.support[0:6])
plt.xticks(list(range(0,6)),freq_item.itemsets[0:6] )
plt.xlabel("movie-sets")
plt.ylabel("support")

rules = association_rules(freq_item, metric="lift", min_threshold=1) # 60 rules
rules.shape
rules.head(20)
...
rules_r  = rules.iloc[index_rules,:] # getting rules without any redudancy, 18 rules
...
plt.plot(rules_r['lift'], rules_r['confidence'], 'rs', rules_r['lift'], fit_fn(rules_r['lift']))
plt.xlabel('lift')
plt.ylabel('confidence')
plt.title('lift vs confidence')

#using different support and length value
freq_item1 = apriori(movie, min_support = 0.25, max_len = 2, use_colnames = True) 
...
#using different support and length value
freq_item2 = apriori(movie, min_support = 0.001, max_len = 3, use_colnames = True) 
...
unique2 = [list(m) for m in set(tuple(i) for i in rules_sets2)] 
index_rules2= []
for i in unique2:
    index_rules2.append(rules_sets2.index(i))
 
rules_r2  = rules2.iloc[index_rules2,:] # getting rules without any redudancy, 0 rules
rules_r2.sort_values('lift',ascending=False).head(10) 

#using different support and length value
freq_item3 = apriori(movie, min_support = 0.5, max_len = 2, use_colnames = True) 
...
rules3 = association_rules(freq_item3, metric="lift", min_threshold=1) # 2 rules
rules3.shape
rules3.head(20)
